...
func TestNew(t *testing.T) {
	...
	w, err := Create(logtool.RLog, p, []byte("somedata"))
	if err != nil {
...
func TestCreateFailFromPollutedDir(t *testing.T) {
	...
	_, err = Create(logtool.RLog, p, []byte("data"))
	if err != os.ErrExist {
...
func TestNewForInitedDir(t *testing.T) {
	...
	if _, err = Create(logtool.RLog, p, nil); err == nil || err != os.ErrExist {
...
func TestOpenAtIndex(t *testing.T) {
	...
	w, err := Open(logtool.RLog, dir, walpb.Snapshot{})
	if err != nil {
		t.Fatalf("err = %v, want nil", err)
	}
	if g := filepath.Base(w.tail().Name()); g != walName(0, 0) {
	...
	w, err = Open(logtool.RLog, dir, walpb.Snapshot{Index: 5})
	if err != nil {
		t.Fatalf("err = %v, want nil", err)
	}
	...
	if _, err = Open(logtool.RLog, emptydir, walpb.Snapshot{}); err != ErrFileNotFound {
...
func TestSaveWithCut(t *testing.T) {
	...
	w, err := Create(logtool.RLog, p, []byte("metadata"))
	if err != nil {
		t.Fatal(err)
	}
	...
	neww, err := Open(logtool.RLog, p, walpb.Snapshot{})
	if err != nil {
		t.Fatalf("err = %v, want nil", err)
	}
	defer neww.Close()
...
func TestRecover(t *testing.T) {
	...
	if w, err = Open(logtool.RLog, p, walpb.Snapshot{}); err != nil {
		t.Fatal(err)
	}
...
func TestSearchIndex(t *testing.T) {
	...
	for i, tt := range tests {
		idx, ok := searchIndex(logtool.RLog, tt.names, tt.index)
		if idx != tt.widx {
...
func TestRecoverAfterCut(t *testing.T) {
	...
	md, err := Create(logtool.RLog, p, []byte("metadata"))
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 10; i++ {
	...
	for i := 0; i < 10; i++ {
		w, err := Open(logtool.RLog, p, walpb.Snapshot{Index: uint64(i)})
		if err != nil {
			if i <= 4 {
...
func TestOpenForRead(t *testing.T) {
	...
	w2, err := OpenForRead(logtool.RLog, p, walpb.Snapshot{})
	if err != nil {
		t.Fatal(err)
	}
	defer w2.Close()
...
func TestRestartCreateWal(t *testing.T) {
	...
	w, werr := Create(logtool.RLog, p, []byte("abc"))
	if werr != nil {
		t.Fatal(werr)
	}
	w.Close()
	...
	if w, err = OpenForRead(logtool.RLog, p, walpb.Snapshot{}); err != nil {
		t.Fatal(err)
	}
	defer w.Close()
...
func TestOpenOnTornWrite(t *testing.T) {
	...
	w, err = OpenForRead(logtool.RLog, p, walpb.Snapshot{})
	if err != nil {
		t.Fatal(err)
	}
